...
@Tag(
        name = "차용증 연동 계좌 API",
        description = "차용증 작성 시 채권자가 드롭다운에서 선택할 수 있는 활성 계좌 조회 API"
)
@RestController
...
public class ContractAccountController {
    ...
    @Operation(
            ...
            description = "본인이 연동한 계좌 중 현재 활성(AVAILABLE) 상태인 계좌 목록을 조회합니다. 차용증 작성 화면의 계좌번호/예금주 드롭다운에 사용됩니다."
    )
    @GetMapping
    public ResponseEntity<List<LinkedBankAccountResponse>> getSelectableAccounts(
        ...
        return ResponseEntity.ok(contractAccountService.getSelectableAccounts(userDetails.getUserId()));
    }
    ...
    @PatchMapping("/{contractId}/changecontractaccount")
    public ResponseEntity<Void> changeContractAccount(
        ...
        CustomUserDetails userDetails = (CustomUserDetails) authentication.getPrincipal();
        contractAccountService.changeContractAccount(contractId, userDetails.getUserId(), request.linkedAccountId());
    ...
    public ResponseEntity<Void> deactivateContractAccount(
        ...
        CustomUserDetails userDetails = (CustomUserDetails) authentication.getPrincipal();
        contractAccountService.deactivateContractAccount(contractId, userDetails.getUserId());
        return ResponseEntity.noContent().build();
    }
}
