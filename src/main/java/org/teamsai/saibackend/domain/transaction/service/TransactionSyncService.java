    ...
    **/
    public int syncTransactions(Long userId, Long linkedAccountId) {
        ...
        List<BankTransactionResponse> transactions = fetchTransactions(
                linkedAccountId, linkedAccount.getAccountId(), userKey, afterTransactionId
        );

        return bankTransactionPersistenceService.saveAndAdvanceCursor(linkedAccountId, transactions);
    }

    private void validateOwnership(Long userId, LinkedBankAccountDTO linkedAccount) {
        if (!linkedAccount.getUserId().equals(userId)) {
            log.warn(
                    "[TransactionSyncService] 소유자가 아닌 계좌 동기화 시도 - requesterId: {}, linkedAccountId: {}",
    ...
    private List<BankTransactionResponse> fetchTransactions(
            Long linkedAccountId, Long bankAccountId, String userKey, long afterTransactionId
    ) {
        try {
            return mockBankClient.getTransactions(bankAccountId, userKey, afterTransactionId);
